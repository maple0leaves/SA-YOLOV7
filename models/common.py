...
class CSPSEConv(nn.Module):
    # Standard convolution
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super(CSPSEConv, self).__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        # self.act = FReLU(c2) if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
    ...
    def forward(self, x):
        return self.cspse(self.act(self.bn(self.conv(x))))

    def fuseforward(self, x):
        return self.cspse(self.act(self.conv(x)))


class SGEConv(nn.Module):
    # Standard convolution
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super(SGEConv, self).__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        # self.act = FReLU(c2) if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
    ...
    def forward(self, x):
        return self.sge(self.act(self.bn(self.conv(x))))

    def fuseforward(self, x):
        return self.sge(self.act(self.conv(x)))
...
class SAConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        ...
        # self.act = AconC(width=c2) if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        self.sa = ShuffleAttention(c2)

    def forward(self, x):
        return self.sa(self.act(self.bn(self.conv(x))))
        # return self.act(self.bn(self.conv(self.sa(x))))

    def fuseforward(self, x):
        return self.sa(self.act(self.conv(x)))
        # return self.sa(self.act(self.conv(self.conv(x))))

# author 221  2023/8/9 create
...
# author 221  2023/8/9 create
class BiformerConv(nn.Module):
    # Standard convolution
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super(BiformerConv, self).__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        self.bif = BiLevelRoutingAttention(c2)
    def forward(self, x):
        return self.bif(self.act(self.bn(self.conv(x))))

    def fuseforward(self, x):
        return self.bif(self.act(self.conv(x)))


# author 221  2023/7/18 create
class EMAConv(nn.Module):
    # Standard convolution
    def __init__(self
                 , c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
    ...
    def forward(self, x):
        return self.ema(self.act(self.bn(self.conv(x))))
        # return self.act(self.bn(self.conv(self.ema(x))))

    def fuseforward(self, x):
        return self.ema(self.act(self.conv(x)))
        # return self.act(self.conv(self.ema(x)))


# author 221  2023/8/2 create
...
# author 221 2023/8/2 add Coodridnate attention (CA)
...
class ShuffleAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # x_channel = x_0 * self.silu(x_channel)

        # spatial attention
        x_spatial = self.gn(x_1)  # bs*G,c//(2*G),h,w
        x_spatial = self.sweight * x_spatial + self.sbias  # bs*G,c//(2*G),h,w
        x_spatial = x_1 * self.sigmoid(x_spatial)  # bs*G,c//(2*G),h,w
        # x_spatial = x_1 * self.silu(x_spatial)  # bs*G,c//(2*G),h,w
...
#   author 221 2023/9/11 add
class CSPSE(nn.Module):
    def __init__(self, channel, reduction=16):
        super(CSPSE, self).__init__()
        self.channle = channel//2

        self.conv1 = Conv(channel,channel//2)#降通道
        self.conv2 = Conv(self.channle,self.channle*2)#升通道
        self.conv3 = Conv(channel*2,channel)
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        #如果有效果，把中间的激活函数换成silu
        self.fc = nn.Sequential(
            nn.Linear(self.channle, self.channle // reduction, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(self.channle // reduction, self.channle, bias=False),
            nn.Sigmoid()
        )
    def forward(self, x):
        x1= self.conv1(x)
        ...
        b, c, _, _ = x1.size()
        # print(self.avg_pool(x1).size())
        y = self.avg_pool(x1).view(b, c) #对应Squeeze操作
        # print(self.fc(y).size())
        y = self.fc(y).view(b, c, 1, 1) #对应Excitation操作
        p = x2*x1.expand_as(x2)
        p = self.conv2(p)
        cat = torch.cat([x,p],dim=1)
        out = self.conv3(cat)

        return out
#   author 221 2023/8/22 add
...
# author 221  add bifpn 2023/11/13
